...
class Library_Borrow(Library_basis):
    async def add_one(
        ...
        try:
            ...
            for view in [
                result_client[0] if result_client else None,
...
Index is invalid"
            ...
            status = [
                self.serialize(view)
                for view in ([response] if type(response) != list else response)
            ]
...
Mistake => Not working index. Index is invalid"
                raise ValueError(text)
            attrib_list = [book_id_, client_id_, date_borrow_, date_return_]
...
Mistake => Object not found, was"
            ...
            if quantity_:
                ...
                if borrow.date_return and (borrow.date_return > borrow.date_borrow):
                    borrow.books.quantity += 1
                elif borrow.date_borrow and quantity_ and not borrow.date_return:
                    borrow.books.quantity -= 1
